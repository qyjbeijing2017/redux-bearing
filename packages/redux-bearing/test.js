...
class TestBearingState extends bearing_state_1.BearingState {
...
// 
const bearing = new bearing_1.default({
    // 你可以直接传入一个基本的BearingState<T>的类型，这样Bearing将会自动生成一个对应T的状态。
    bearingVal1: bearing_state_1.BearingState.fromDefaultVal(1),
    // 可以通过fromDefaultVal方法创建一个BearingState，这样Bearing将会自动生成一个对应T的状态，并且将默认值设置成你传递的值。
    bearingVal2: bearing_state_1.BearingState.fromDefaultVal('val2'),
    // Bearing同样支持树结构的状态，虽然不推荐这样做，但是为了兼容之前的Redux树，或者为了其他什么事情，我们还是提供了这样的方式。
    bearingVal3: {
        bearingVal31: bearing_state_1.BearingState.fromDefaultVal(31),
        bearingVal32: bearing_state_1.BearingState.fromDefaultVal('val32'),
    ...
    val1: bearing_state_1.BearingState.fromDefaultVal('val1'),
    val2: {
        val21: bearing_state_1.BearingState.fromDefaultVal('val21'),
        val22: bearing_state_1.BearingState.fromDefaultVal('val22'),
    },
}, 
// 第二个参数可以填入之前工程设定的reducer，这将让Bearing和传统的Redux还有其他框架一起工作，当然如果你不想要这个功能，或者这是一个新的工程，这个参数可以不填。
{
    reducer: (0, redux_1.combineReducers)({
        val1: reducer1,
        val2: (0, redux_1.combineReducers)({
            val21: reducer2,
            val22: reducer3
        })
    })
...
// 传统的redux事件。
store.dispatch((0, exports.testAction)());
// 这时生成的state如下
// {
//   bearingVal1: null,
//   bearingVal2: null,
...
// test default value
const bearingTestDefaultValue = new bearing_1.default({
    val1: bearing_state_1.BearingState.fromDefaultVal('hello'),
});
